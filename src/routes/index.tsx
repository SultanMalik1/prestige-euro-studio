            ...
            <div className="border border-border bg-background p-6 sm:p-9 lg:col-span-7">
              ...
              {submitted ? (
                ...
                <form onSubmit={submitForm} className="mt-8 space-y-5" noValidate>
                  ...
                  <div className="grid gap-5 sm:grid-cols-2"><Field label="Vehicle" name="vehicle" placeholder="Year, make, model" required /><label className="block"><span className="form-label">Service</span><select key={formMode} name="service" className="form-field" defaultValue={formMode === "quote" ? "Body Shop & Paint" : "Advanced Diagnostics"}>{services.map((service) => <option key={service.title}>{service.title}</option>)}</select></label></div>
